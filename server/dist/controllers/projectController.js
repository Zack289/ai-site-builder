...
// Controller function to make code revision
export const makeRevision = async (req, res) => {
    ...
    try {
        ...
        //user needs at 5 least credits for revision
        if (user.credits < 5) {
            return res
                .status(403)
        ...
        const enhancedPrompt = promptEnhancePrompt.choices[0].message.content;
        await prisma.conversation.create({
            data: {
                role: "assistant",
...
export const rollbackToVersion = async (req, res) => {
    try {
        ...
        if (Array.isArray(projectId) || Array.isArray(versionId)) {
...
export const getPublishedProojects = async (req, res) => {
    try {
