  ...
  const handleRollBack = async (versionId: string) => {
    try {
      const confirm = window.confirm(
        "Are you sure want to roll back to this version?",
      );
      if (!confirm) return;
        ...
        <form onSubmit={handleRevisions} className="m-3 relative">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:indigo-700 text-white transition-colors disabled:opacity-60"
            >
              {isGenerating ? (
